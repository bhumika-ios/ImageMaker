    ...
    var point: CGPoint?
     ...
     override func touchesMoved(_ touches: Set<UITouch>, with event: UIEvent?) {
         ...
         if let event = event {
             ...
             // TODO: To make underlaying canvas quality better try to wait for state .began and the add all delayed move events
              /*if mlCanvas?.drawingGestureRecognizer.state == .began || mlCanvas?.drawingGestureRecognizer.state == .changed {
                 mlCanvas?.drawingGestureRecognizer.touchesMoved(touches, with: event)
              }*/
         }
         ...
         // start timer to detect user hold for 1 second
         timer?.invalidate()
         timer = Timer.scheduledTimer(withTimeInterval: 1.5, repeats: false) { _ in
             self.mlCanvas?.state = .recognizing
             
             // finish current touches with success - ML will be called after
             if let event = event {
